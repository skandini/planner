...
export function getEventPositionStyles(
  ...
  if (useClassicCascade && totalColumns > 1) {
    ...
    // Улучшенная формула для более выразительного каскада
    const baseWidth = 92; // Увеличена базовая ширина для лучшего заполнения
    const offsetPx = column * cascadeOffset; // Смещение в пикселях
    
    // Добавляем тень для лучшей видимости наслоения
    return {
      left: `${offsetPx}px`,
